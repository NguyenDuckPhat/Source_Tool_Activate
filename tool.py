...
class NguyenDucPhat_Activated_Page:
    ...
    def banner(self):
        ...
        [  TOOL KÍCH HOẠT PAGE PRO5 VIP PRO - VERSION 1.0  ]
        '''
        print(banner)
    def ndp_delay_tool(self, p):
        while(p>1):
            p=p-1
            print(f'[NGUYENDUCPHAT][|][LO......][{p}]','     ',end='\r');sleep(1/6)
            print(f'[NGUYENDUCPHAT][/][LOA.....][{p}]','     ',end='\r');sleep(1/6)
            print(f'[NGUYENDUCPHAT][-][LOAD....][{p}]','     ',end='\r');sleep(1/6)
            print(f'[NGUYENDUCPHAT][+][LOADI...][{p}]','     ',end='\r');sleep(1/6)
            print(f'[NGUYENDUCPHAT][\][LOADIN..][{p}]','     ',end='\r');sleep(1/6)
            print(f'[NGUYENDUCPHAT][|][LOADING.][{p}]','     ',end='\r');sleep(1/6)
    ...
    def Activated_Page(self, cookie: str, fb_dtsg: str, jazoest: str, lsd: str, uid_page_die):
        ...
        try:
            dem+=1
            print(f'[NĐP] | {dem} | ACTIVATED_PAGE_SUCCESS | [NAME_PAGE: '+Json_Activated_Page['data']['reactivate_profile']['name']+'] | [UID_PAGE: '+Json_Activated_Page['data']['reactivate_profile']['id']+']')
        except:
            print(f'[NĐP] | {dem} | ACTIVATED_PAGE_ERROR | [UID_PAGE: {uid_page_die}]')
# =========================== [ START TOOL ] ===========================         
